...
class AdaptiveAveragePooling(Module):

    """docstring for AdaptiveAveragePooling"""
    ...
    def update_output(self, x):
        ...
        strideH = H // self.oH
        strideW = W // self.oW
        pool_height = H - (self.oH-1)*strideH
        pool_width = W - (self.oW-1)*strideW

        self.kW, self.kH = pool_height, pool_width
        self.dW, self.dH = strideW, strideH

        assert (
            H - pool_height) % strideH == 0 or H == pool_height, 'Invalid height'
        assert (
            W - pool_width) % strideW == 0 or W == pool_width, 'Invalid width'

        out_height = int(np.floor((H - pool_height) // strideH + 1))
        out_width = int(np.floor((W - pool_width) // strideW + 1))

        x_split = x.reshape(N * C, 1, H, W)
        x_cols = im2col_cython(
            x_split, pool_height, pool_width, padding=0, stride=strideW)
