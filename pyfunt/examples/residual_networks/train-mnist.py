...
def main():
    ...
    test_set = MNIST(DATASET_PATH, train=False, download=True)

    # print(type(train_set.data.numpy()))
    # print((train_set.data.numpy()).shape)
    # print((np.expand_dims(train_set.data.numpy(), axis=1)).shape)
    # print(np.pad(np.expand_dims(train_set.data.numpy(), axis=1), (0,0,2,2), 'constant').shape)

    data = {
        'X_train': (np.pad(np.expand_dims(train_set.data.numpy(), axis=1), [(0, 0), (0, 0), (2, 2), (2, 2)], 'constant') / 127.5 - 1)[:1000],
        'y_train': np.array(train_set.targets),
        'X_val': np.pad(np.expand_dims(test_set.data.numpy(), axis=1), [(0, 0), (0, 0), (2, 2), (2, 2)], 'constant') / 127.5 - 1,
        'y_val': np.array(test_set.targets),
    }
